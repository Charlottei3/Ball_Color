...
public class IAPManager : SingletonMonoBehaviour<IAPManager>, IStoreListener, IDetailedStoreListener
{
    ...
    private void InitializePurchasing()
    {
        ...
        foreach (IAPPack pack in packs)
        {
            ...
            builder.AddProduct(pack.androidId, pack.type);
    ...
    public void OnInitialized(IStoreController controller, IExtensionProvider extensions)
    {
        ...
        foreach (IAPPack pack in packs)
        {
...
#if UNITY_ANDROID || UNITY_EDITOR
            product = storeController.products.WithID(pack.androidId);
#elif UNITY_IOS
            product = storeController.products.WithID(pack.apple_id_pack);
#endif
            if (product != null)
            {
                pack.packagePrice = product.metadata.localizedPrice.ToString();
                pack.currencyCode = product.metadata.isoCurrencyCode;
#if UNITY_ANDROID
                Debug.Log("[IAPManager]: Pack: " + pack.androidId + " avaiable" + "Purchase avaiable " + product.availableToPurchase + " Price: " + pack.packagePrice);
#elif UNITY_IOS
            ...
            {
#if UNITY_ANDROID
                Debug.LogError("[IAPManager]: Pack: " + pack.androidId + " unavaiable");
#elif UNITY_IOS
